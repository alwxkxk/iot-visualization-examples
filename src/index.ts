...
// for orbit reset
let main: Object3dWrap;

// for reset action
let racks: Object3dWrap[];
let servers: Object3dWrap[];

//  for lock outline
let lockRack: Object3dWrap;
let lockServer: Object3dWrap;
...
const updateRightInfo = (() => {
  ...
  return (rack: Object3dWrap, server?: Object3dWrap) => {
    if (rack) {
...
function clickServer(results: any[]) {
  if (results.length > 0) {
    const server = results[0].object.$Object3dWrap.raycasterRedirect;
    lockServer = server;
    // reset other
    servers.forEach((c) => {
      c.resetAction();
...
function showServerModel(rack: Object3dWrap) {
  ...
  servers.forEach((c: Object3dWrap) => {
...
function moveRack(results: any[]) {
  if (results.length > 0) {
    const rack = results[0].object.$Object3dWrap.raycasterRedirect;
...
function dblclickRack(results: any[]) {
  if (results.length > 0) {
    lockRack = results[0].object.$Object3dWrap.raycasterRedirect;
